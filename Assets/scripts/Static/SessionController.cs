...
[System.Serializable] public class Session
{
    ...
    #region // Map Cell information
        ...
        public delegate float FormulaUp(int x, int y, byte floor, ulong key);
        public delegate PlatformPreset FormulaMod(int x, int y, byte floor, ulong key);
        public delegate int FormulaLet(int x, int y, byte floor, ulong key);
        ...
        [System.Serializable] struct MapCell
        {
            ...
            public MapCell(Checkers Position, PlatformPreset Mod){ 
                
                position = Position;
                effect = new List<MapEffect>();
                DeformProtection = Mod.DeformProtection;

                AttackVisualize = GameObject.Instantiate(AttackVisualizer, Position.ToVector3(true), AttackVisualizer.transform.rotation, AttackVisualizerFolder(position.layer).transform);
